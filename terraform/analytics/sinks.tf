...
# 4 本目の Splunk は Spark を通さず Kafka の sink（MSK Connect の Splunk Connect for Kafka）にする予定で、まだ作らない（docs/phases.md）
...
resource "aws_opensearchserverless_security_policy" "logs_network" {
  ...
  name        = local.logs_collection
  type        = "network"
  description = "Collection reachable only through the VPC endpoint of terraform/analytics"
