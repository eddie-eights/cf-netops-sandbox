...
variable "name_prefix" {
  description = "Same value as terraform/ecr / terraform/main (ECR repository names are derived from it)."
  type        = string
...
variable "containerlab_version" {
  description = "containerlab_<version>_linux_arm64.rpm must be uploaded to s3://<kb_bucket_name of terraform/main>/lab/"
  type        = string
  default     = "0.79.0"
...
variable "telegraf_version" {
  description = "Phase 2. telegraf-<version>-1.aarch64.rpm in s3://<kb_bucket_name of terraform/main>/lab/ is installed when present (README s-1). Nothing happens without it."
  type        = string
  default     = "1.40.0"
