...
output "lab_security_group_id" {
  description = "Read by terraform/stream (Kafka 9098 from the lab EC2)"
  value       = aws_security_group.lab.id
}

output "lab_role_name" {
  description = "Read by terraform/stream (kafka-cluster write permissions are attached to this role)"
