...
# ---------------------------------------------------------------- read by terraform/agent, terraform/lab, terraform/stream, terraform/analytics, terraform/graph, terraform/workflow (terraform_remote_state)
output "vpc_id" {
  description = "Read by terraform/agent / terraform/lab / terraform/stream / terraform/graph / terraform/workflow"
  value       = aws_vpc.this.id
}

output "runtime_subnet_ids" {
  description = "Read by terraform/agent (runtime ENIs and endpoints), terraform/stream (MSK brokers) and terraform/graph (Neptune subnet group)"
  value       = [aws_subnet.a.id, aws_subnet.b.id]
}

output "instance_subnet_id" {
  description = "Read by terraform/lab (the lab EC2 sits next to the chat web) and terraform/agent (bedrock-agentcore endpoint)"
...
output "endpoint_security_group_id" {
  description = "Read by terraform/agent (its interface endpoints use this SG) and terraform/lab / terraform/stream, which add 443 ingress rules so the lab EC2 and MSK Connect can reach the endpoints"
  value       = aws_security_group.endpoints.id
}

output "kb_bucket_name" {
  description = "Read by terraform/agent (docs/), terraform/lab (lab/) and terraform/stream (stream/)"
  value       = aws_s3_bucket.kb.bucket
}

output "kb_bucket_arn" {
  description = "ARN of the bucket (read by terraform/agent / terraform/lab / terraform/stream for IAM policies)"
  value       = aws_s3_bucket.kb.arn
}

output "runtime_role_name" {
  description = "Read by terraform/agent (execution policy), terraform/stream / terraform/graph (read policies for the anomaly table and Neptune)"
...
output "web_role_name" {
  description = "Read by terraform/agent (InvokeAgentRuntime) / terraform/stream / terraform/graph"
