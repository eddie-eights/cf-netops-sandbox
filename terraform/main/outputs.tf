...
output "upload_web_command" {
  description = "Run in this repository after \"pip download\" into wheels/ (README step 4). Also copies the agent modules the web UI shares (topology / anomalies / graph). The instance pulls web/ on every boot."
  value       = "aws s3 cp web/app.py s3://${aws_s3_bucket.kb.bucket}/web/app.py && aws s3 cp web/requirements.txt s3://${aws_s3_bucket.kb.bucket}/web/requirements.txt && for f in topology anomalies graph; do aws s3 cp agent/$f.py s3://${aws_s3_bucket.kb.bucket}/web/$f.py; done && aws s3 cp agent/data/ s3://${aws_s3_bucket.kb.bucket}/web/data/ --recursive && aws s3 sync wheels/ s3://${aws_s3_bucket.kb.bucket}/web/wheels/"
}
