...
# The Spark job of terraform/analytics puts AnomalyOpened / AnomalyResolved on the default bus when a link goes down / comes back.
...
resource "aws_iam_role" "status" {
  ...
  description        = "Status Lambda of terraform/graph - writes the dynamic status of devices and links into Neptune"
...
resource "aws_cloudwatch_event_rule" "status" {
  name        = "${var.name_prefix}-graph-status"
  description = "AnomalyOpened / AnomalyResolved from the Spark job (terraform/analytics) to the status Lambda"
