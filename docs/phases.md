...
番号は **1 / 2 / 2B / 5** を使う。3 は欠番、4（設定履歴・手順書のベクトル + 全文検索）は**フェーズ 1 に取り込んだ**ので単独では存在しない。
...
これに合わせて **2B（保存層と分析）を足し**、フェーズ 5（修復）は **pending**（当面は人が承認する形のまま）にした。
**まだ作っていないのは 2B とフェーズ 5 の 2 つで、どちらも下に「決まっていないこと」を書いてある。**
...
| 5 | 異常から「分析して」→ 承認 → 修復 → 検証を一周させる | 未着手 | 未試算 | **pending**（人が承認する形のまま。2026-09-16 決定） |
...
- **閉域から呼べるか。これが最初の確認事項。**この VPC には IGW も NAT も無く、外に出る経路は VPC エンドポイントだけ。
  **S3 Tables / EMR Serverless / Glue にインターフェースエンドポイントがあるかは未確認**（2026-09-16 時点。AWS の認証が切れていて引けていない）。
  無ければこの構成からは呼べないので、置き場から考え直すことになる。
- **エンドポイントが要る場合の固定費。**インターフェースエンドポイントは 1 つにつき $0.014/h/AZ（1 AZ で月 ≒ $10、2 AZ で月 ≒ $20）。
  いまでもエンドポイントだけで 15 AZ 時間 = $0.21/h（月 ≒ $153）出ている。**「従量だから置いても 0」はこの構成では成り立たない。**
...
## フェーズ 5 — 承認して直す（pending）

**このリポジトリには入っていない。**コードも Terraform も無い。
**2026-09-16 のユーザー決定で、ネットワークの自動修復は一旦 pending。**当面は**人が承認する（HITL）**形のままにする。

### 決まっていること（考え方）

- 異常一覧から「分析して」→ **人が承認** → 修復 → 検証、までを一周させる。**自動では直さない。**
...
### ワークフローのエンジン（方針は決まった。着手は pending）

- 2026-09-16 の「道具を揃える」決定で **Temporal** に寄せる。同日に**置き場は EKS**と決まった（AgentCore と連携させたいため）。
- ただし**修復そのものが pending** なので、**このリポジトリにはまだ EKS を足さない。**着手するときに入れる。
  それまでの間、承認付きのワークフローが要るなら元の案（**Step Functions Standard**。承認待ちに `.waitForTaskToken` が要り、
  Express は Request Response しか使えない）で足りる。

**EKS を入れるときに効く費用と前提**（着手前にこの 3 つを確かめる）

| 見るもの | いま分かっていること |
|---|---|
| クラスタ本体 | **$0.10/h（月 ≒ $73）**。東京・2026-09-16 に確認。フェーズ 1 の 約 $0.52/h に対して **+19%** で、**止めない限り毎時かかる** |
| ノード | EKS Fargate は x86 $0.05056/vCPU 時間 + $0.00553/GB 時間（同日確認）。EC2 ノードにするなら別途。Temporal はデータベースも要る |
| 閉域から使えるか | **未確認。**この VPC には IGW も NAT も無い。いまあるインターフェースエンドポイントは `ecr.api` / `ecr.dkr` / `logs` / `bedrock-runtime` / `bedrock-agent-runtime` / `ssm` / `ssmmessages` / `bedrock-agentcore` / `lambda` / `sts` で、**EKS に何が足りないかは AWS の文書で確認していない。**足すぶんは 1 つにつき月 ≒ $10〜$20 |

- 毎日 `ops/down.sh` で消す運用なので、**EKS も「使う日に作って当日中に消す」側に入れる。**置きっぱなしにすると、
  クラスタ本体だけでフェーズ 2（stream + graph）の月額の 4 分の 1 近くになる。
