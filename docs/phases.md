...
- 2026-09-16 の「道具を揃える」決定で **Temporal** に寄せる方針になった。
- ただし**修復そのものが pending** なので、着手するときに Temporal で組むか、元の案（**Step Functions Standard**。
  承認待ちに `.waitForTaskToken` が要り、Express は Request Response しか使えない）で組むかを決める。
- **Temporal を置くなら lab の EC2（t4g.large）の docker compose に同居させる。追加費用は 0。**
  ECS Fargate に置くと 1 vCPU 2 GB で月 ≒ $36 + データベースぶんかかる（東京・2026-09-16 に確認）。
  同居させるなら、2 vCPU / 8 GB に containerlab と一緒に収まるかを先に見る。
