...
| 閉域から使えるか | **動く。ただしエンドポイントが要る**（AWS は「外に出られないクラスタ」の作り方を文書にしている。2026-09-16 に確認）。条件は 2 つ: クラスタのエンドポイントを private にすること、イメージを VPC 内の ECR から引くこと |
| 足りないエンドポイント | **4〜5 個足りない。**文書が挙げるのは `ec2` / `ecr.api` / `ecr.dkr` / `s3`（ゲートウェイ）/ `elasticloadbalancing` / `logs` / `sts` / `oidc-eks`（IRSA）/ `eks-auth`（Pod Identity）/ `eks` / `ssm`（任意）/ `xray`（任意）。main 側にあるのは `ecr.api` / `ecr.dkr` / `logs` / `ssm` / `ssmmessages` と S3 ゲートウェイだけなので、**`ec2` / `sts` / `eks` と、`oidc-eks` か `eks-auth` のどちらかが足りない**（ロードバランサを使うなら `elasticloadbalancing` も）。**1 AZ で月 ≒ $40〜$50、2 AZ で月 ≒ $80〜$100** |
