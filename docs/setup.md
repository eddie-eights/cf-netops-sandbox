...
この一式のコマンドは全部 bash 用なので、Windows でも **WSL2 の中で打てば下の手順 7 の PowerShell の注意（クォートの違い）は関係ない**。Windows 側に入れた aws / terraform / docker は WSL からは見えないので、全部 WSL 側に入れる。WSL で足りているか、次を見る。
