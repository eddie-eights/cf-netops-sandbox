...
| 症状 | 原因と直し方 |
|---|---|
| 異常一覧には出るのに、トポロジも調査ワークフローも動かない（`ops/up.sh` を打ち直した直後） | 古い Spark のジョブが動いたまま。`ops/up.sh` は動いているジョブを起こし直さないので、`cancel-job-run`（[pipeline.md](pipeline.md) の「変えたとき」）してから `ops/up.sh` |
