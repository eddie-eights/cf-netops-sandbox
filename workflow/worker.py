...
ワークフローの段: get_anomaly → investigate（AgentCore Runtime に JSON で答えさせる）→ put_proposal（pending）
  ...
  → approved なら apply_on_lab（SSM Run Command で `sudo lab <cmd>`。cmd は rules.ALLOWED_ACTIONS だけ）→ applied / failed
...
async def put_proposal(anomaly: dict, finding: dict, wf_id: str) -> str:
    ...
    await asyncio.to_thread(awsio.write_proposal, {
...
async def start_for(client: Client, anomaly: dict) -> bool:
    ...
    existing = await asyncio.to_thread(awsio.read_proposal, anomaly.get("anomaly_id", ""))
    if not rules.should_start(anomaly, existing):
        return False
    try:
        await client.start_workflow(InvestigateAnomaly.run, anomaly["anomaly_id"],
                                    id=rules.workflow_id(anomaly["anomaly_id"]), task_queue=TASK_QUEUE)
        log.info("started %s", rules.workflow_id(anomaly["anomaly_id"]))
        return True
...
async def starter_queue(client: Client) -> None:
    ...
    for m in await asyncio.to_thread(awsio.receive_messages):
        aid = rules.anomaly_id_from_message(m.get("Body", ""))
        if aid:
